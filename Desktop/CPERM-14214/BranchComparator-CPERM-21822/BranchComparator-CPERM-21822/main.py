...
def home():
    ...
    if results:
        ...
        #print(type(issues))
        issues_r=[]
        jql1=",".join(issues).split(",")
        for each in jql1:
            issues_r.append(each)
        ...
        filtername=(report_file_name.split("/")[1]).strip("xls")[0]
        for each in issues_r:
            try:
                jf=jira.issue(each)
            except :
                print("jql1")
                print(issues_r)
                print(type(issues_r))
                i=issues_r.index(each)
                del issues_r[i]

        jql = 'issue in (' + ', '.join(issues_r)+ ')'
        jf=jira.create_filter(name=filtername,jql=jql)
        results['report_file_name'] = report_file_name
        results['all_results'] = jf.id
    else:
...
def compare_release_tags():
    ...
    if results:
        ...
        filter_name=report_file_name
        jira=jira_login()
        filtername=(report_file_name.split("/")[1]).strip("xls")[0]
        jf=jira.create_filter(name=filter_name,jql=jql)
        results['report_file_name'] = report_file_name
        results['all_results'] = jf.id
    else:
        print('\n*** No changes ***\n')
